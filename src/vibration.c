...
/* * EMA FILTER (0.1)
 * - Smoothing factor of 0.1 removes noise.
 * - Preserves Gravity/Tilt (True Values).
 */
#define EMA_ALPHA 0.1

/*
 * --- CALIBRATION VALUES ---
 * Update these with your idle readings from ThingsBoard.
 * Vertical Mount: Y is Gravity.
 */
...
static void accel_trigger_handler(const struct device *dev, const struct sensor_trigger *trig)
{
    ...
    if (sensor_channel_get(dev, SENSOR_CHAN_ACCEL_XYZ, data) < 0) return;
    ...
    if (filtered_sample[0] == 0.0 && filtered_sample[1] == 0.0) {
        ...
        filtered_sample[0] = (EMA_ALPHA * raw_x) + (1.0 - EMA_ALPHA) * filtered_sample[0];
        filtered_sample[1] = (EMA_ALPHA * raw_y) + (1.0 - EMA_ALPHA) * filtered_sample[1];
        filtered_sample[2] = (EMA_ALPHA * raw_z) + (1.0 - EMA_ALPHA) * filtered_sample[2];
    }
}
...
void start_vibration_tracking(void)
{
    if (device_is_ready(accel_sensor)) {
        sensor_trigger_set(accel_sensor, &accel_sensor_trigger, accel_trigger_handler);
    } else {
